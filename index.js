...
async function identify_user(agent)
  ...
  const snap = await client.db("chatbot").collection("user_table").findOne({acct_num: `${num}`});
  if(snap==null){
...
function report_issue(agent)
{
  ...
  const intent_val=agent.parameters.issue;
  ...
  MongoClient.connect(url, function(err, db) {
    ...
    var time_date=year + "-" + month + "-" + date;
	  ...
	  var obj = { username:u_name, issue:issue_val,status:status,time_date:time_date,trouble_ticket:trouble_ticket };

    dbo.collection("user_issues").insertOne(obj, function(err, res) {
      if (err) throw err;
      db.close();    
    });
 });
...
intentMap.set("service_intent-custom",report_issue );
intentMap.set("service_intent-custom-custom",custom_payload);
